...
import { getJob, closeJob } from "@/modules/jobs/job.service";
...
export async function POST(
  ...
  try {
    ...
    await closeJob(id, auth.membership.membership_id);

    return NextResponse.json({ job_id: id, status: "closed" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Internal server error";
    let status = 500;
    if (message.includes("permission")) status = 403;
    if (message.includes("already closed")) status = 400;
